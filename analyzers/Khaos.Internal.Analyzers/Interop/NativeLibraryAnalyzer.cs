...
public sealed class NativeLibraryAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeMethod(SyntaxNodeAnalysisContext context, ImmutableArray<INamedTypeSymbol> libraryImport)
    {
        ...
        if (Find(method.GetAttributes(), libraryImport) is not AttributeData attribute)
            return;
    ...
    private static AttributeData? Find(ImmutableArray<AttributeData> attributes, ImmutableArray<INamedTypeSymbol> candidates)
