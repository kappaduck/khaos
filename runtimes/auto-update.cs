...
const string path = "runtimes/utils/sdl.env";
...
if (!File.Exists(path))
{
    Console.Error.WriteLine($"Cannot find the SDL versions at '{path}'");
    return 1;
}

using HttpClient http = CreateGitHubClient();

Span<string> lines = await File.ReadAllLinesAsync(path);
...
for (int i = 0; i < lines.Length; i++)
{
    ...
    line = $"{key}={latest}";
    updates.Add(new UpdatedRelease(key, repository, current, latest));
}
...
Console.WriteLine($"Updated {updates.Count} SDL {(updates.Count == 1 ? "library" : "libraries")} in '{path}'.");
return 0;
